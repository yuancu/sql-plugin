...
public class CalcitePPLBuiltinFunctionIT extends CalcitePPLIntegTestCase {
  ...
  public void testAbs() {
    JSONObject actual =
            executeQuery(
                    String.format(
                            "source=%s | where age = abs(-30) | fields name, age",
                            TEST_INDEX_STATE_COUNTRY));
  ...
  public void testAbsWithField() {
    JSONObject actual = executeQuery(
            String.format(
                    "source=%s | where abs(age) = 30 | fields name, age",
                    TEST_INDEX_STATE_COUNTRY)
    );
  ...
  public void testLn() {
    JSONObject actual =
            executeQuery(
                    String.format(
                            "source=%s | where ln(age) > 4 | fields name, age",
                            TEST_INDEX_STATE_COUNTRY));
  ...
  public void testMod() {
    JSONObject actual =
            executeQuery(
                    String.format(
                            "source=%s | where mod(age, 10) = 0 | fields name, age",
                            TEST_INDEX_STATE_COUNTRY));
